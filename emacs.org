...
** Emacsclient Support For StumpWM
This was taken from https://github.com/stumpwm/stumpwm/wiki/WorkWithEmacsclient. It is set up to be triggered only on my
Gentoo setup, so if you're using this you will definitely need to change things.

#+BEGIN_SRC emacs-lisp
  (add-hook 'after-init-hook 'server-start)
  (setq server-raise-frame t)

  ;(if (and window-system (string-equal system-name "kelleyld-w541"))
