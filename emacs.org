...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-agenda-files (list "~/planner.org" "~/classes.org"))
#+END_SRC
...
I like using Org’s clocking function, but I want to change the time it takes
before it yells at me for being away from Emacs for too long. I also prefer
editing the clocked time by the minute than by every 5.
