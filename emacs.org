...
#+BEGIN_SRC emacs-lisp
  (req-package stumpwm-mode
    :init
    (setq stumpwm-shell-program "~/scripts/stumpish")
...
#+BEGIN_SRC emacs-lisp
    ...
    (setq org-agenda-files (list (concat org-dir "planner.org")
                                 (concat org-dir "classes.org")
                                 (concat org-dir "calendar.org")))
    (setq org-columns-default-format
          "%20ITEM(Task) %8Effort(Estimated){:} %CLOCKSUM")
#+END_SRC

Org babel things. You’ll probably want to change the location of the plantuml
jar file, but who knows, it might be right! I also ask org not to bug me about
evaluating code blocks in files; it’s just not worth the hassle, especially when
I’m exporting a large notes file.
...
I like using Org’s clocking function.
