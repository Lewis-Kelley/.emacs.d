...
Thanks to a missing feature in =battery.el=, the battery function I
use, =battery-linux-sysfs= doesn't include the =%b= option for no
discernible reason. I've gone ahead and added some advice to fix the
issue.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun sysfs-add-summary (data)
    (let* ((percentage (string-to-number (cdr (assoc ?p data))))
           (verbose-status (downcase (cdr (assoc ?B data))))
           (status-symbol
            (cond
             ((string= verbose-status "charging") "+")
             ((< percentage battery-load-critical) "!")
             ((< percentage battery-load-low) "-")
             (t ""))))
      (cons (cons ?b status-symbol) data)))

  (advice-add 'battery-linux-sysfs :filter-return 'sysfs-add-summary)
#+END_SRC
* Packages
** Control
...
#+BEGIN_SRC emacs-lisp
  (use-package dad-joke
    ...
    :bind (("s-d" . dad-joke)))
#+END_SRC
*** [[https://gitlab.petton.fr/DamienCassou/desktop-environment][desktop-environment]]
#+BEGIN_SRC emacs-lisp
...
Used to keep Emacs's idea of PATH in line with my =.bashrc= settings.
...
#+BEGIN_SRC emacs-lisp
  (use-package exwm
    :custom (exwm-workspace-number 9)
    :bind
    ("M-&" . (lambda (command)
               (interactive (list (read-shell-command "$ ")))
               (start-process-shell-command command nil command)))
    :hook
    (exwm-update-class . simplify-exwm-class)
    ;; (exwm-update-title . simplify-exwm-title)
    :init
    (defun preserved-exwm-instance-namep (exwm-instance-name)
      (or (string-prefix-p "sun-awt-X11-" exwm-instance-name)
          (string= "gimp" exwm-instance-name)))

    (defun simplify-exwm-class ()
      (unless (preserved-exwm-instance-namep exwm-instance-name)
        (exwm-workspace-rename-buffer exwm-class-name)))

    (defun simplify-exwm-title ()
      (unless (preserved-exwm-instance-namep exwm-instance-name)
    ...
    (defun launch-exwm (arg)
      (when (equal arg "exwm")
        ...
        (exwm-enable)
        ;; Because the workspaces each need scroll-bar-mode turned off
        (scroll-bar-mode -1)))
